# Release Checklist

- [x] GitHub repository is public or otherwise accessible to reviewers.
- [x] README explains the manuscript, folder structure, and reproduction steps.
- [x] CITATION.cff exists and includes the release version metadata.
- [x] LICENSE exists.
- [x] data/ contains material coefficients and shareable input assumptions.
- [x] results/ contains the 80-case matrix, PRCC diagnostics, Monte Carlo convergence diagnostics, and sensitivity outputs.
- [x] figures/figure_source_data/ contains source data for all main figures.
- [x] scripts/ or notebooks/ can reproduce the tables/figures claimed as reproducible.
- [x] docs/proprietary_data_note.md explains the FAWT non-disclosure boundary.
- [x] GitHub release v1.0.4-submission exists.
- [x] Zenodo has archived the GitHub release.
- [x] Zenodo DOI resolves publicly.
- [x] Manuscript Data availability contains the GitHub URL and Zenodo DOI.
- [x] No unused or unresolved DOI remains in the active manuscript or submission files.
